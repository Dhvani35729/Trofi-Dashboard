...
urlpatterns = [
    ...
    re_path(r'api/restaurant/(?P<restaurant_id>[\w,-]+)/hours/(?P<hour_id>[0-9][0-9]{0,1}?)/menu/$',
            views.api_restaurant_menu, name='apiRestaurantMenu'),
    re_path(r'api/users/(?P<user_private_id>\w+)/order/$',
            views.api_user_order, name='apiUserOrder'),
    ...
    re_path(r'api/users/(?P<user_private_id>\w+)/card/$',
            views.api_user_card, name='apiUserCard'),
    re_path(r'api/users/(?P<user_private_id>\w+)/card/default$',
            views.api_user_change_default_card, name='apiUserChangeDefaultCard'),
    re_path(r'api/users/(?P<user_private_id>\w+)/card/add$',
